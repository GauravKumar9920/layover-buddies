...
-- ============================================================================
-- Keep auth.users and public app tables in sync without requiring client-side
-- inserts that are blocked by RLS.
...
DECLARE
  ...
  derived_name TEXT;
BEGIN
  provider_text := COALESCE(NEW.raw_app_meta_data->>'provider', 'email');
  provider_value := CASE
  ...
  VALUES (
    NEW.id,
    COALESCE(NEW.email, NEW.id::text || '@mumbai-buddies.local'),
    derived_name,
    'traveler'::user_role,
    provider_value,
...
-- Helper keeps sync logic reusable for backfills and explicit client-triggered sync.
