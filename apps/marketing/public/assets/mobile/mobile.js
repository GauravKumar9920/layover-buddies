(() => {
  ...
  let state = {hours:8,mood:'coast',places:[],fields:{},checks:[],solid:false};
  ...
  function update(){
    ...
    shell.classList.toggle('mw-solid',state.solid);
    const glass=document.querySelector('[data-mw-glass]');glass.setAttribute('aria-pressed',String(state.solid));glass.textContent=state.solid?'Restore glass':'Reduce glass';
  }
  ...
  document.querySelector('[data-mw-glass]').addEventListener('click',()=>{state.solid=!state.solid;update();save()});
  ...
  form.addEventListener('submit',async event=>{
    ...
    try {
      ...
      if(result.fallback)$('mw-success').querySelector('p').textContent='Your request was sent through our backup email service. Your hosts will review it and reply by email. Your trip is not confirmed yet.';
  ...
  document.querySelector('[data-mw-clear]').addEventListener('click',()=>{if(sending){toast('Please wait for your request to finish sending.');return;}state={hours:8,mood:'coast',places:[],fields:{},checks:[],solid:state.solid};resetRequest();toast('Your saved plan and draft details have been cleared.')});
