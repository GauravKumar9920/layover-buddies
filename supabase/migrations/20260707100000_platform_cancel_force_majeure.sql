-- ============================================================================
-- FIX: platform/system cancellations must use force-majeure economics
-- ============================================================================
-- The original resolver (migration 20260512100300) lumped p_actor='system'
-- into the traveler-voluntary branch: a platform- or ops-initiated
-- cancellation of a paid booking landed in the gt_72h/24_to_72h/lt_24h tiers
-- and could forfeit the traveler's deposit (or convert their trip pot to a
-- voucher) for a decision the platform made.
--
-- The mobile state machine already routes platform/system cancel actors to
-- cancelled_force_majeure (full refunds both sides, late fee waived, nobody
-- penalised, no buddy ban). This migration brings the DB resolver — the copy
-- that actually moves money — in line, and accepts 'platform' as an actor.
--
-- Unchanged behaviour:
--   * cron paths — t_minus_12_no_pay / deposit_window_expired triggers are
--     matched before the actor branch, exactly as before.
--   * traveler and buddy voluntary cancellations — identical tiers.
--
-- Mirrors: apps/mobile/lib/booking/cancellationSnapshot.ts (updated in the
-- same change) and supabase/functions/_shared/stateMachine.ts.
-- ============================================================================
