...
//
// Canonical reducer shared by mobile and edge functions. Deposit recovery is
// an explicit guarded event; persisted transitions remain atomic SQL commands.
// ============================================================================
...
export type BookingEvent =
  ...
  | { kind: 'dispute_resolved'; outcome: 'resume' | 'settled' | 'cancel'; previousState?: 'trip_ready' | 'in_progress' | 'awaiting_proofs' | 'reconciling' | 'completed' | 'rated' }
  ...
  | { kind: 'no_show_marked'; party: 'traveler' | 'buddy' } // party = who FAILED to show
...
const TRANSITIONS = new Map<BookingState, TransitionRule[]>([
  ...
  ['deposits_held', [{ event: 'deposits_recovered', guard: (_, ctx) => ctx.bothDepositsHeld, next: 'awaiting_balance' }]],
  ...
  // Reports pause the trip for review; only an admin confirms a no-show.
  // The SQL commands enforce the two-hour grace period.
  ['trip_ready', [
    { event: 'no_show_reported', next: 'disputed' },
    { event: 'qr_scanned', next: 'in_progress' },
    {
      event: 'no_show_marked',
      next: (e) => e.kind === 'no_show_marked' && e.party === 'buddy'
        ? 'no_show_buddy'
        : 'no_show_traveler',
    },
    {
  ...
  ['completed', [
    { event: 'dispute_raised', guard: (_, ctx) => ctx.disputeWindowOpen === true, next: 'disputed' },
    { event: 'rating_submitted', next: 'rated' },
  ]],

  // ── Terminal states ────────────────────────────────────────────────────────
  // Rated bookings may reopen inside the reporting window.
  ['rated', [{ event: 'dispute_raised', guard: (_, ctx) => ctx.disputeWindowOpen === true, next: 'disputed' }]],
  ['disputed', [
    { event: 'dispute_resolved', next: (e) => e.kind === 'dispute_resolved' && e.outcome === 'resume' ? (e.previousState ?? 'reconciling') : e.kind === 'dispute_resolved' && e.outcome === 'cancel' ? 'cancelled_force_majeure' : 'completed' },
  ]],
  ['cancelled',                   []],
  ['cancelled_no_pay',            []],
