...
const TRANSITIONS = new Map<BookingState, TransitionRule[]>([
  ...
  ['deposits_held', []],
  // Platform/system cancellations route to cancelled_force_majeure in every
  // post-deposit state: routing them to cancelled_traveler_voluntary would
  // apply the traveler's penalty tiers (voucher / forfeiture) to a
  // cancellation the traveler didn't choose. Must match
  // mobile/lib/booking/stateMachine.ts.
  ['awaiting_balance', [
    ...
    { event: 'cancel', guard: (e) => e.kind === 'cancel' && (e.actor === 'platform' || e.actor === 'system'), next: 'cancelled_force_majeure' },
